...
const fileUpload = async (req = request, res = response) => {
  ...
  const tiposValidos = ["hospitales", "doctores", "usuarios"];

  if (!tiposValidos.includes(tipo)) {
    return res.status(400).json({
      ok: false,
      msg: "El tipo selecionado no es un doctores, usuarios o hospital (tipo)",
    });
  }
  ...
  if (!extensionesValida.includes(extensionArchivo)) {
    return res.status(400).json({
      ok: false,
      msg: "Formato de imagen invalido, solo son permitidos los formatos png,jpg,jpeg y gif",
    });
  }
